...
{
    ESP_LOGI("status_handler", "Entered the function");
    ...
    *p++ = 0;
    ...
    ESP_LOGI("status_handler", "Exiting the function");  // 함수 끝 부분에 로그 추가
...
esp_err_t feed_handler(httpd_req_t *req) {
    ESP_LOGI("feed_handler", "Entered the function");  // 함수 시작 부분에 로그 추가
    ...
    if (httpd_req_recv(req, buf, remaining) < 0) {

        ESP_LOGE("feed_handler", "Failed to receive the request");
    ...
    if(err) {
        ESP_LOGE("feed_handler", "deserializeJson() failed: %s", err.c_str());
        return ESP_FAIL;
    }

    // deserializeJson(doc, buf); 
    int delayTime = doc["timer"] | 0; // timer 값을 밀리초로 받아옵니다.

    myservo.attach(2);

    if(!myservo.attached()) {
        ESP_LOGE("feed_handler", "Failed to attach the servo");
       return ESP_FAIL;
    }

    // 원하는 각도로 서보 모터를 회전시킨다.
    myservo.write(180); // 예를 들어, 180도로 설정했습니다.
    delay(delayTime); // timer 값 만큼 delay
    myservo.write(0); // 초기 위치로 돌아옵니다.

    myservo.detach();

    httpd_resp_sendstr(req, "Servo moved");

    ESP_LOGI("feed_handler", "Servo moved");  // 서보 이동 로그 추가
    return ESP_OK;
...
// 새로운 URI 핸들러 추가
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        // 새로운 URI 핸들러를 camera_httpd 서버에 등록
        httpd_register_uri_handler(camera_httpd, &feed_uri);  
    }
